# portfolio-No-004: PHP BBS (Login System)

## 概要
PHPとMySQLを使用した、ユーザー認証機能付きの掲示板アプリケーションです。
「誰が、いつ、何を投稿したか」をデータベースで厳密に管理し、セッションによるログイン状態の維持や、管理者・一般ユーザーの権限分けなど、Webアプリケーションの基本となるバックエンド処理の実装に重点を置いています。  
...
https://github.com/z-at-0/portfolio-No-004/issues/1
...
- **技術要素**: 
    - PDOによるDB接続・操作
    - セッション管理（session_start）
    - 権限（Role）ベースのアクセス制御
- **開発環境**: XAMPP (Apache, MySQL)
- 
## 設計方針
処理の役割ごとにPHPファイルを分割し、「ログイン」「投稿処理」「投稿取得」「削除処理」などを  それぞれ独立させることで、コードの見通しを良くすることを意識しました。
...
## 動作環境
本アプリケーションはPHPとMySQLを使用しているため、ローカル開発環境（XAMPP）で動作確認を行っています。
...
### ■ 補足
004は、私の開発学習において「バックエンドの基礎」を固める重要なステップとなりました。AIを活用してエラー解決の時間を短縮しつつも、コアとなるDB操作やセッションの仕組みについては、自分の手で動かしながら深い理解を得ることに注力しています。
